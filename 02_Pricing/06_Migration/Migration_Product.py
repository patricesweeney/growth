...
def calculate_new_revenue(data, p1, v1, p2, weibull_params, product_column):
    ...
    for product in data[product_column].unique():
        ...
        if lambda_param and rho_param:
            ...
            data.loc[data[product_column] == product, 'lift'] = np.log(data.loc[data[product_column] == product, 'r2'] / data.loc[data[product_column] == product, 'r1'])
...
def calculate_impact(new_data):
    ...
    # Revenue Impact
    total_revenue_diff = (new_data['p2'] * new_data['v2']).sum() - (new_data['p1'] * new_data['v1']).sum()
    percent_revenue_diff = total_revenue_diff / (new_data['p1'] * new_data['v1']).sum()

    # Average Price per Row (APRA) Impact
    apra_diff = (new_data['p2'] * new_data['v2']).mean() - (new_data['p1'] * new_data['v1']).mean()
    percent_apra_diff = apra_diff / (new_data['p1'] * new_data['v1']).mean()
    ...
    # Seats Impact
    seats_diff = new_data['v2'].sum() - new_data['v1'].sum()
    percent_seats_diff = seats_diff / new_data['v1'].sum()

    # Average Revenue per Unit (ARPU) Impact
    arpu_diff = ((new_data['p2'] * new_data['v2']) / new_data['v2']).mean() - ((new_data['p1'] * new_data['v1']) / new_data['v1']).mean()
    percent_arpu_diff = arpu_diff / ((new_data['p1'] * new_data['v1']) / new_data['v1']).mean()
    ...
    # Plot distribution of lift column
    plt.figure()
    plt.hist(new_data['lift'].dropna(), bins=20, edgecolor='black')
    ...
    results = {
        ...
        '\nSeats Difference': f"{seats_diff:,d}",
